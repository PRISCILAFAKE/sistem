...
try {
    $sql = "SELECT r.*, 
                   ts.nombre_s AS nombre_tipo_solicitud, 
                   urg.nombre_ur AS nombre_urgencia, 
                   p.estado,
                   u.id_usuario,
                   tr.nombre AS nombre_usuario,
                   ar.nombre_area AS nombre_area_usuario 
            FROM requerimientos r 
            LEFT JOIN tipo_solicitud ts ON r.id_tipo_solicitud_fk = ts.id_tipos 
            ...
            LEFT JOIN trabajador tr ON u.id_trabajador_fk = tr.id_trabajador 
            LEFT JOIN areas ar ON tr.id_area_fk = ar.id_area 
    ...
    $stmt_conteo = $conexion->prepare("SELECT COUNT(*) as total FROM requerimientos WHERE id_usuario_f = :id_usr");
    $stmt_conteo->execute([':id_usr' => $id_usuario]);
    $resultado_conteo = $stmt_conteo->fetch(PDO::FETCH_ASSOC);
    $cantidad_requerimientos = $resultado_conteo['total'] ?? 1;
    ...
    $numeroOrden = str_pad($cantidad_requerimientos, 2, '0', STR_PAD_LEFT);
...
if ($es_jefe == 1) {
    $linkVolver = "bandeja_jefe_view.php";
} elseif ($rol_usuario === 'gerencia' || $rol_usuario === 'gerente') {
    $linkVolver = "solicitud_ge_view.php";
} else {
    $linkVolver = "solicitud_crud_view.php";
}
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <title>Vista de Requerimiento - Detalle</title>
...
<div class="container mt-4 mb-4" style="max-width: 800px;">
    <div class="card shadow-sm p-4 border-0">
        ...
        <div class="border-bottom pb-3 mb-4">
            <div class="row align-items-center">
                <div class="col-8">
                    ...
                    <p class="text-muted small m-0">Detalle del Requerimiento</p>
                </div>
            ...
            <div class="mt-1">
                <?php if (!empty($requerimiento['archivo_adjunto'])): ?>
                    ...
                    <div class="ocultar-en-impresion">
                        ...
                        <?php elseif ($esPdf): ?>
                            <div>
                                <iframe src="<?php echo $rutaArchivo; ?>" width="100%" height="300px" class="border rounded"></iframe>
                            </div>
                        <?php else: ?>
        ...
        <div class="d-flex justify-content-end gap-2 no-print">
            <a href="<?php echo $linkVolver; ?>" class="btn btn-secondary btn-sm"><i class="bi bi-arrow-left"></i> Volver a la Bandeja</a>
        </div>
    </div>
</div>
